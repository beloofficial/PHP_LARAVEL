      ...
      @foreach($posts as $k=>$post)
    	<div class="post w3-white" style="display: flex;" >
      		<div class="post_left" style="width: 15%">
      			<a href="{{route('another_user_sub',['login'=>$post->login])}}"><img class="post_icon" src="{{asset('assets/images/'.$post->icon)}}" ></a>
      		</div>
      		<div class="post_right" style="width: 85%">
      			<a href="{{route('another_user_sub',['login'=>$post->login])}}"><h3 class="post_login">{{$post->login}}</h3></a>
      			<p class="post_date">{{$post->created_at}}</p>
      			<hr>
      			<p class="post_text">{!!$post->text!!}</p>
      			<br>
      			<img class="post_img"  src="{{asset('assets/img/'.$post->img)}}">
      			<br>
